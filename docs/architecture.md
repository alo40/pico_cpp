...
* Selects the intended synchronized dataset explicitly rather than silently
  assuming that the most recently modified CSV is the desired analysis input.
* Preserves the timezone-aware logger-host local timestamp by default; timezone
  conversion is an explicit analysis operation rather than an implicit loading
  step.
* Is expected to run cleanly from top to bottom before its outputs are treated
  as a reproducible analysis result.
...
analyst must synchronize data before analyzing it and deliberately select the
measurement file or files to analyze.
...
The first inspection notebook now implements the initial explicit single-file
analysis interface. It resolves the project-relative `data/processed/`
directory, requires a visible filename selection, rejects paths outside that
directory, and does not select input by filesystem modification time. It
contains no synchronization or network operations.

Notebook text and implementation agree that the CSV timestamp is a
timezone-aware logger-host local receive/processing timestamp. Parsing preserves
the stored offset and does not convert it to UTC. Any future UTC or other
timezone conversion must remain explicit and documented.

The saved notebook outputs were generated by one clean top-to-bottom execution
with sequential execution counters. The selected file,
`vedirect_2026-09-05_141727.csv`, contains 12,648 samples over approximately
3.5 hours with no detected sequence gaps, missing sequences, duplicates, or
restart transitions. Its PV power is mostly zero and peaks at 2 W, so it does
not provide meaningful daylight production evidence.

The selected timestamped session is useful for verifying the notebook interface
and analyses, but it is not evidence of the current Raspberry Pi daily-file
acquisition path. Validation against an explicitly identified Raspberry Pi
daily file with meaningful daylight production and the complete disconnect ->
acquire -> reconnect -> synchronize -> analyze workflow remain open.
...
hardware timestamp or a Pico acquisition timestamp. The analysis layer should
preserve this timezone-aware local timestamp by default. Reading a value such as
`2026-09-06T00:00:00.892+02:00` and retaining the `+02:00` offset is not the
same as converting it to UTC; any UTC conversion must be explicit. Adjacent
values may contain unmeasured jitter from USB transport, Raspberry Pi
scheduling, Python execution, and serial buffering, so they are not a precision
hardware clock.
